...
csvFile = open( '삼성전자주가.csv' , 'r' )# 2.  해당 csv 파일 오픈
...
for row in list(csvContent)[ 1 : ] : # 4. 읽어온 csv 파일 확인하기 , 리스트로 타입변환 ,첫번째 행 제거(슬라이싱 이용한 제거)
    ...
    날짜리스트.append( row[0].split("/")[1] +'/'+ row[0].split("/")[2] )      # 날짜리스트에 날짜 대입 , 연도 제거 , 월/일 표현
    가격리스트.append( int( row[1] ) )      # 가격리스트에 가격 대입 , 정수 타입 변환
